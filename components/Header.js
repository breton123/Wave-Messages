...
function Header() {
  ...
  useEffect(
    () =>
      onSnapshot(
        query(collection(db, "users", userID, "friends")),
  ...
  return (
    <div
      className={`${
        colorMode ? "bg-[#222831] text-white" : "bg-base-300 text-gray-800"
      }  flex flex-col font-medium px-3  text-center items-center py-2 shadow-lg space-y-5 ${
        size ? "w-40" : "w-20"
      }`}
    >
      <DotsCircleHorizontalIcon
        className={`absolute h-6  cursor-pointer ${
          size ? "left-40" : "left-20"
        }`}
        onClick={() => ChangeSideBar()}
      />
      {!session && (
        <h1 className="mb-5 cursor-pointer" onClick={signIn}>
          Login
        </h1>
      )}
      {session && (
        <h1 className="mb-5 cursor-pointer" onClick={signOut}>
          Log Out
        </h1>
      )}
      <div className="flex justify-between space-x-2 items-center cursor-pointer">
        <img
          className={`bg-blue-500 rounded-full ${size ? "h-10" : "h-20"}`}
      ...
      <div className="flex flex-col items-center">
        ...
        />
        {colorMode ? (
          <h1 className={`${size ? "" : "hidden"}`}>Dark Mode</h1>
        ) : (
          <h1 className={`${size ? "" : "hidden"}`}>Light Mode</h1>
        )}
      </div>
    </div>
  );
